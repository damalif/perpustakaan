                    ...
                    <div class="x_content">
                        <form action="<?= base_url($target_url) ?>" method="post" enctype="multipart/form-data">
                            ...
                            <div class=" field item form-group">
                                ...
                                <div class="input-group col-md-6 col-sm-6">
                                    <input type="number" name="publication_year" class="form-control <?= (!empty(session()->getFlashdata('validation')['publication_year'])) ? 'is-invalid' : '' ?>" placeholder="" value="<?= ($is_edit) ? $item['publication_year'] : old('publication_year') ?>">
                                    <div class="invalid-feedback">
                            ...
                            <div class=" field item form-group">
                                ...
                                <div class="input-group col-md-6 col-sm-6">
                                    <select class="form-control <?= (!empty(session()->getFlashdata('validation')['id_category'])) ? 'is-invalid' : '' ?>" name="id_category">
                                        <option value=""></option>
                                        <?php foreach ($category as $poin) : ?>
                                            <option <?= ($poin['id'] == old('id_category')) ? 'selected' : (($is_edit) ? (($poin['id'] == $item['id_category']) ? 'selected' : '') : '') ?> value="<?= $poin['id'] ?>"><?= $poin['category'] ?></option>
                                        <?php endforeach ?>
                                        <div class="invalid-feedback">
